...
// IncomingPaymentLookupLayer: Orchestrates lookup popups and suggestions for payment filtering.
...
const INCOMING_PAYMENT_BREADCRUMB = {
  href: "/sales/incoming-payment",
  page: "Incoming Payments Data Table",
  section: "Sales",
} as const;
...
export interface IncomingPaymentLookupLayerProps {
  tableId: string;
  table: ReturnType<typeof useReactTable<IncomingPaymentListItem>>;
  onReset: () => void;
  onCreateClick: () => void;
}

export function IncomingPaymentLookupLayer({
  tableId,
  table,
  onReset,
  onCreateClick,
}: IncomingPaymentLookupLayerProps) {
  ...
  const docNumSuggestionsQuery = useQuery(
    incomingPaymentQueries.docNumSuggestions(undefined, DOC_NUM_QUICK_LIMIT),
  );
  const docNumSuggestionsBackgroundQuery = useQuery({
    ...incomingPaymentQueries.docNumSuggestions(undefined, DOC_NUM_BACKGROUND_LIMIT),
  ...
  const docNumLookupSearchQuery = useQuery({
    ...incomingPaymentQueries.docNumSuggestions(
  ...
  return (
    <>
      <TableToolbar
        ...
        createLink="/sales/create-incoming-payment"
        breadcrumb={INCOMING_PAYMENT_BREADCRUMB}
