...
// SalesQuotationLookupLayer: Orchestrates lookup popups and suggestions for sales quotation filtering.
...
const SALES_QUOTATION_BREADCRUMB = {
  href: "/sales/quotations",
  page: "Sales Quotations Data Table",
  section: "Sales",
} as const;
...
export interface SalesQuotationLookupLayerProps {
  tableId: string;
  table: ReturnType<typeof useReactTable<SalesQuotationListItem>>;
  onReset: () => void;
  onCreateClick: () => void;
}

export function SalesQuotationLookupLayer({
  tableId,
  table,
  onReset,
  onCreateClick,
}: SalesQuotationLookupLayerProps) {
  ...
  const docNumSuggestionsQuery = useQuery(
    salesQuotationQueries.docNumSuggestions(undefined, DOC_NUM_QUICK_LIMIT),
  );
  const docNumSuggestionsBackgroundQuery = useQuery({
    ...salesQuotationQueries.docNumSuggestions(undefined, DOC_NUM_BACKGROUND_LIMIT),
  ...
  const docNumLookupSearchQuery = useQuery({
    ...salesQuotationQueries.docNumSuggestions(
  ...
  return (
    <>
      <TableToolbar
        ...
        createLink="/sales/create-quotation"
        breadcrumb={SALES_QUOTATION_BREADCRUMB}
