...
const PREFETCH_RECENT_MS = 15_000;
const PREFETCH_WASTE_WINDOW_MS = 120_000;
...
const sweepRecent = (now: number) => {
  for (const [key, ts] of localRecentPrefetch) {
    if (now - ts > PREFETCH_RECENT_MS) {
      localRecentPrefetch.delete(key);
    }
  }
  for (const [key, ts] of crossTabRecentPrefetch) {
    if (now - ts > PREFETCH_RECENT_MS) {
      crossTabRecentPrefetch.delete(key);
    }
  }
};
...
const publishCrossTabPrefetch = (serializedKey: string, at: number) => {
  ...
  prefetchChannel?.postMessage({ at, key: serializedKey });
};
...
const exposeMetrics = () => {
  ...
  const target = window as Window & {
    __TABLE_PREFETCH_METRICS__?: () => PrefetchMetrics & {
      hitRate: number;
      wasteRate: number;
    };
  };
...
const ensureQueryCacheObserver = (queryClient: QueryClient) => {
  if (hasAttachedCacheObserver) {
    return;
  }
  hasAttachedCacheObserver = true;

  queryClient.getQueryCache().subscribe((event) => {
    if (!event || event.type !== "observerAdded") {
      return;
    }
    ...
    if (!prefetchedButUnconsumed.has(serializedKey) || consumedKeys.has(serializedKey)) {
      return;
    }
