export type TableFilterType = "text" | "number-comparison" | "date" | "select" | "boolean";
...
export const normalizeDateRange = (range: DateRangeFilter): DateRangeFilter => {
  if (!range.from || !range.to) {
    return toDateRangeFilter(range.from, range.to);
  }
...
export const matchesNumberComparison = (rowValue: unknown, filterValue: unknown): boolean => {
  const cleanRow =
    typeof rowValue === "string" ? rowValue.replaceAll(/[^0-9.-]/g, "") : String(rowValue ?? "");
  const parsedRow = Number.parseFloat(cleanRow);
  if (!Number.isFinite(parsedRow)) {
    return false;
  }
  ...
  if (!operator || !Number.isFinite(parsed)) {
    return true;
  }
