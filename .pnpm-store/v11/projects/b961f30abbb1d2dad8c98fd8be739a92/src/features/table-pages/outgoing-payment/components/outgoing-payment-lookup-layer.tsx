...
// OutgoingPaymentLookupLayer: Orchestrates lookup popups and suggestions for payment filtering.
...
export interface OutgoingPaymentLookupLayerProps {
  tableId: string;
...
export function OutgoingPaymentLookupLayer({
  ...
  const docNumSuggestionsQuery = useQuery(
    outgoingPaymentQueries.docNumSuggestions(undefined, DOC_NUM_QUICK_LIMIT),
  );
  const docNumSuggestionsBackgroundQuery = useQuery({
    ...outgoingPaymentQueries.docNumSuggestions(undefined, DOC_NUM_BACKGROUND_LIMIT),
