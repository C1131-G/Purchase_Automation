...
// ARInvoiceLookupLayer: Orchestrates lookup popups and suggestions for accounts receivable filtering.
...
const AR_INVOICE_BREADCRUMB = {
  href: "/sales/ar-invoice",
  page: "AR Invoices Data Table",
  section: "Sales",
} as const;
...
export interface ARInvoiceLookupLayerProps {
  tableId: string;
...
export function ARInvoiceLookupLayer({
  ...
  const docNumSuggestionsQuery = useQuery(
    arInvoiceQueries.docNumSuggestions(undefined, DOC_NUM_QUICK_LIMIT),
  );
  const docNumSuggestionsBackgroundQuery = useQuery({
    ...arInvoiceQueries.docNumSuggestions(undefined, DOC_NUM_BACKGROUND_LIMIT),
  ...
  const docNumLookupSearchQuery = useQuery({
    ...arInvoiceQueries.docNumSuggestions(
