...
router.get("/", async (req, res, next) => {
  try {
    ...
    const result = await arInvoiceService.getInvoices(dbName, {
...
router.get("/docnums", lookupLimiter, async (req, res, next) => {
  try {
    const dbName = getDbName(req);
    const result = await arInvoiceService.getInvoiceDocNums(
      dbName,
...
router.get("/:id", async (req, res, next) => {
  try {
    const dbName = getDbName(req);
    const data = await arInvoiceService.getInvoice(dbName, req.params.id);
    if (!data) {
