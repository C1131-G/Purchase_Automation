...
router.get("/", async (req, res, next) => {
  try {
    ...
    const result = await incomingPaymentService.getPayments(dbName, {
...
router.get("/docnums", lookupLimiter, async (req, res, next) => {
  try {
    const dbName = getDbName(req);
    const result = await incomingPaymentService.getPaymentDocNums(
      dbName,
...
router.get("/by-doc-num/:docNum", async (req, res, next) => {
  try {
    const dbName = getDbName(req);
    const data = await incomingPaymentService.getPaymentByDocNum(dbName, req.params.docNum);
    if (!data) {
...
router.get("/:id", async (req, res, next) => {
  try {
    const dbName = getDbName(req);
    const data = await documentService.getDocument(dbName, "IncomingPayment", req.params.id);
    if (!data) {
      return res.status(404).json({ message: "Payment not found", success: false });
    }
    res.status(200).json({ data, success: true });
