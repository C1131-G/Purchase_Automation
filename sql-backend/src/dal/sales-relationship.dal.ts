// Sales Relationship DAL: Express handler for document relationship tracing.

import type { RequestHandler } from "express";
import { relationshipMapService, type DocType } from "@/services/relationship-map.service";
...
export const getRelationshipMap: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!Number.isInteger(entry) || entry <= 0) {
      return res
        .status(400)
        .json({ message: "docEntry must be a positive integer", success: false });
    }
