...
export const getGRPOs = async (
  ...
  const repo = ds.getRepository(GRPOSchema);
...
export const getGRPODocNums = async (dbName: string, search?: string, limit = 10) => {
  const ds = await getTenantDataSource(dbName);
  const repo = ds.getRepository(GRPOSchema);
...
export const getGRPO = async (dbName: string, id: string) => {
  const ds = await getTenantDataSource(dbName);
  const repo = ds.getRepository(GRPOSchema);
  return repo.findOne({ where: { docEntry: Number.parseInt(id) } });
};

export const getAvailablePOs = async (dbName: string, search?: string, limit = 10) => {
  const ds = await getTenantDataSource(dbName);
  const repo = ds.getRepository(PurchaseOrderSchema);

  const where: Record<string, unknown> = { docStatus: "O" };
  if (search) {
    where.cardCode = Like(`%${search}%`);
  }
