...
import { cancelInvoice } from "@/modules/ap-invoice/ap-invoice.update-cancel.mutations";
...
describe("cancelInvoice", () => {
  ...
  it("posts cancel to Service Layer", async () => {
    ...
    const result = await cancelInvoice("sess-1", "42");

    expect(request).toHaveBeenCalledWith("sess-1", "POST", "/PurchaseInvoices(42)/Cancel");
  ...
  it("rethrows when Service Layer fails", async () => {
    ...
    await expect(cancelInvoice("sess-1", "42")).rejects.toThrow("SAP down");
  });
});
