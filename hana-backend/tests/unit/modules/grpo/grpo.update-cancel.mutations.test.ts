...
import { cancelGRPO } from "@/modules/grpo/grpo.update-cancel.mutations";
...
describe("cancelGRPO", () => {
  ...
  it("posts cancel to Service Layer", async () => {
    ...
    const result = await cancelGRPO("sess-1", "42");

    expect(request).toHaveBeenCalledWith("sess-1", "POST", "/PurchaseDeliveryNotes(42)/Cancel");
  ...
  it("rethrows when Service Layer fails", async () => {
    ...
    await expect(cancelGRPO("sess-1", "42")).rejects.toThrow("SAP down");
  });
});
