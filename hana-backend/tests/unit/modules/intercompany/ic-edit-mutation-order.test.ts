/**
 * ic-edit-mutation-order.test.ts: IC edit preflight ordering — lock before side effects.
 * Covers: PQ/PO/SQ locked rejects skip attachments + SAP PATCH.
 */
...
// Covers: IC lock runs before attachments/SAP in mutations.
describe("IC edit preflight ordering", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  // Verifies locked PQ blocks before attachments/SAP PATCH.
  ...
  // Verifies locked PO blocks before attachments/SAP PATCH.
  ...
  // Verifies IC-created SQ blocks before attachments/SAP PATCH.
