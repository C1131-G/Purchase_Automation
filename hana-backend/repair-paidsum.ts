...
async function repairPaidSum() {
  ...
  try {
    const rows = await executeTenantQuery(
      db,
      'SELECT "DocEntry", "DocNum", "PaidSum" FROM "OINV" WHERE "DocStatus" = \'O\'',
    );
    ...
    for (const fix of fixes) {
      ...
      await executeTenantQuery(
        db,
        `UPDATE "OINV" SET "PaidSum" = "PaidSum" - ${fix.deduction} WHERE "DocNum" = ${fix.docNum} AND "PaidSum" >= ${fix.deduction}`,
      );
      count++;
    }
