...
c.connect((e) => {
  c.exec(
    "SELECT COLUMN_NAME FROM SYS.COLUMNS WHERE TABLE_NAME='IGN1' AND COLUMN_NAME LIKE 'U_%'",
    (e, r) => {
      console.log("IGN1 UDFs:", r);
      c.exec(
        "SELECT COLUMN_NAME FROM SYS.COLUMNS WHERE TABLE_NAME='OIGN' AND COLUMN_NAME LIKE 'U_%'",
        (e, r2) => {
