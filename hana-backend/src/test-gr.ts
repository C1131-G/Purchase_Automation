...
async function run() {
  const loginRes = await axios.post(
    "https://172.18.30.114:50000/b1s/v1/Login",
    {
  ...
  const binRows = await executeTenantQuery(
    "AJAX_POS_DB",
    `SELECT TOP 1 "AbsEntry" FROM OBIN WHERE "WhsCode" = 'L101'`,
  );
  const binEntry = (binRows as any[])[0]?.AbsEntry || 2;
  ...
  try {
    const result = await axios.post(
      "https://172.18.30.114:50000/b1s/v1/InventoryGenEntries",
      payload3,
      {
