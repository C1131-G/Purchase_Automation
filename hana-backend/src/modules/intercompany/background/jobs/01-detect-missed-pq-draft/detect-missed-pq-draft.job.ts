 ...
 */
export type MissedPqDraftSource = {
  listForCompany: (company: IcCompany) => Promise<IcPqDraftHookInput[]>;
};

export const createEmptyMissedPqDraftSource = (): MissedPqDraftSource => ({
  listForCompany: async () => [],
});

export type DetectMissedPqDraftResult = {
  companies: number;
  processed: number;
  created: number;
  skipped: number;
...
export type DetectMissedPqDraftJob = {
  run: () => Promise<DetectMissedPqDraftResult>;
};
 ...
 */
export const createDetectMissedPqDraftJob = (deps?: {
  sql?: IcSqlClient;
  company?: CompanyService;
  configuration?: ConfigurationService;
  flow1?: Flow1Orchestrator;
  source?: MissedPqDraftSource;
  scheduler?: SchedulerService;
}): DetectMissedPqDraftJob => {
  ...
  const source = deps?.source ?? createEmptyMissedPqDraftSource();
  ...
  return {
    run: async () => {
      ...
      const result: DetectMissedPqDraftResult = {
        companies: 0,
        created: 0,
        failed: 0,
        processed: 0,
...
export const detectMissedPqDraftJob = createDetectMissedPqDraftJob();
