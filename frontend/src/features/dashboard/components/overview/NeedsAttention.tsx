    ...
    >
      <div className="border-b border-amber-200 bg-amber-50 px-5 py-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h2 className="text-sm font-semibold tracking-tight text-ink-900">Open AR drafts</h2>
            {hasMoreHint ? (
              <p className="mt-1 text-xs text-neutral-500">Scroll for more · infinite list</p>
            ) : (
          ...
          {totalCount > 0 ? (
            <span className="shrink-0 rounded-lg bg-amber-500 px-2 py-1 text-[11px] font-semibold tabular-nums text-surface">
      ...
      {isError && items.length === 0 ? (
        ...
        >
          <p className="text-sm font-medium text-ink-900">Couldn&apos;t load open AR drafts.</p>
          <button
            type="button"
            onClick={() => void refetch()}
            className="mt-3 self-start rounded-lg bg-amber-500 px-3 py-1.5 text-xs font-semibold text-surface hover:bg-amber-600"
          >
            Retry
          </button>
        </div>
        ...
        <div
          className={cn("flex flex-1 flex-col justify-center px-5 py-8", overviewMotionClass.empty)}
        >
          <p className="text-sm font-medium text-ink-900">No open AR drafts.</p>
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <div className="hidden shrink-0 grid-cols-[minmax(0,5.5rem)_minmax(0,1fr)_minmax(0,6.5rem)_minmax(0,4rem)_minmax(0,5rem)] gap-2 border-b border-amber-100 px-5 py-2.5 text-[11px] font-semibold uppercase tracking-wide text-amber-700/50 sm:grid">
            <span>Doc</span>
            <span>Customer</span>
          ...
          >
            {showVirtual ? (
                ...
                {virtualRows.map((virtualRow) => {
                  ...
                  return (
                    <div
                      ...
                      className="absolute top-0 left-0 grid w-full grid-cols-1 gap-1.5 border-b border-amber-100 px-5 py-3.5 sm:grid-cols-[minmax(0,5.5rem)_minmax(0,1fr)_minmax(0,6.5rem)_minmax(0,4rem)_minmax(0,5rem)] sm:items-center sm:gap-2 sm:py-0"
                      style={{
                    ...
                    >
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium tabular-nums text-ink-900">
                          {docLabel(item)}
                        </p>
                      ...
                      <div className="min-w-0">
                        ...
                        {showCode ? (
                          <p className="mt-0.5 truncate text-[11px] text-neutral-400">{code}</p>
                        ) : item.requester ? (
                          <p className="mt-0.5 truncate text-[11px] text-neutral-400">
                            By {item.requester}
                          </p>
                      ...
                      <p className="text-sm font-medium tabular-nums text-ink-900 sm:text-right">
                        {formatCurrency(item.docTotal, currency)}
                      </p>

                      <p className="text-sm tabular-nums text-neutral-600 sm:text-right">
                        <span className="text-xs text-neutral-400 sm:hidden">Age · </span>
                        {item.ageDays}d
                      </p>

                      <div className="sm:text-right">
                        <span className="inline-flex rounded-lg bg-amber-100 px-2 py-0.5 text-[11px] font-semibold text-amber-800 ring-1 ring-amber-200">
                          {item.status || "Draft"}
                        </span>
                      </div>
                    </div>
              ...
              <div className="flex items-center justify-center px-5 py-10">
                <span className="text-xs font-medium text-amber-700">Loading drafts…</span>
              </div>
            )}
          </div>
          {isFetchingNextPage ? (
            <p className="border-t border-amber-100 bg-amber-50 px-5 py-2 text-center text-[11px] text-amber-700">
              Loading more drafts…
            </p>
          ) : hasNextPage ? (
            <p className="border-t border-amber-100 bg-amber-50 px-5 py-2 text-center text-[11px] text-amber-700">
