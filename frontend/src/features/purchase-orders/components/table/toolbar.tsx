...
export function TableToolbar<TData>({
    ...
    const filterableColumnIds = useMemo(
        () => table.getAllLeafColumns().filter((column) => column.getCanFilter()).map((column) => column.id),
        [table],
    )
    const firstAppliedFilterId = useMemo(() => {
        const isNonEmptyFilterValue = (value: unknown) => {
            ...
            if (value && typeof value === 'object') {
                ...
                if ('operator' in candidate || 'value' in candidate) return candidate.value !== undefined && candidate.value !== null
                return false
            }
            return value !== '' && value !== null && value !== undefined
        }

        const first = tableColumnFilters.find((filter) =>
            filterableColumnIds.includes(filter.id) && isNonEmptyFilterValue(filter.value),
        )
