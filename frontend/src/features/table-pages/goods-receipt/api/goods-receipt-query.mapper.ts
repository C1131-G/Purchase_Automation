...
export const mapSearchToGoodsReceiptListParams = (
  search: GoodsReceiptSearch,
  ...
  const taxStart = taxDate?.from ?? taxDate?.to;
  const taxEnd = taxDate?.to ?? taxDate?.from;
  ...
  const sortBy =
    firstSort && SORTABLE_FIELDS.has(firstSort.id)
      ? (firstSort.id as GoodsReceiptListParams["sortBy"])
      : undefined;
  ...
  return {
    ...
    DocStatus: getStringFilter(filters, "DocStatus"),
    DocDateStart: start || search.DocDateStart,
    DocDateEnd: end || search.DocDateEnd,
    TaxDateStart: taxStart || search.TaxDateStart,
    TaxDateEnd: taxEnd || search.TaxDateEnd,
