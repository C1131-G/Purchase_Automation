...
import { mapSearchToTransferListParams } from "@/features/table-pages/transfer/api/transfer-query.mapper";
...
import { createTransferColumns } from "@/features/table-pages/transfer/components/transfer-columns";
import { TransferLookupLayer } from "@/features/table-pages/transfer/components/transfer-lookup-layer";
...
const routeApi = getRouteApi("/_layout/inventory/transfer");
const TABLE_ID = "transfer";
const DEFAULT_COLUMN_ORDER = ["DocNum", "DocDate", "Filler", "ToWhsCode", "DocTotal", "DocStatus"];
...
export function TransferTable() {
  ...
  const columns = useMemo(() => createTransferColumns(), []);
  ...
  const columnFilters = useMemo<ColumnFiltersState>(() => {
    ...
    if ((searchParams as any).DocStatus)
      built.push({ id: "DocStatus", value: (searchParams as any).DocStatus });
  ...
  const listParams = useMemo(() => mapSearchToTransferListParams(searchParams), [searchParams]);
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery(transferQueries.list(listParams));

  const rows = useMemo(() => tList?.data ?? [], [tList?.data]);
  const totalRows = tList?.total ?? 0;
  const totalPages = Math.max(tList?.totalPages ?? 1, 1);
  const showInitialSkeleton = isLoading && !tList;

  // eslint-disable-next-line react-hooks/incompatible-library
  const table = useReactTable<TransferListItem>({
    ...
    onColumnOrderChange: (updater) => {
      ...
      navigate({
        replace: true,
        search: (prev: TransferSearch) => ({ ...prev, columnOrder: [...next] }),
      });
    },
  ...
  useEffect(() => {
    ...
    navigate({
      replace: true,
      search: (prev: TransferSearch) => ({ ...prev, page: clampedPageIndex + 1 }),
    });
  }, [pagination.pageIndex, maxPageIndex, filteredTotalRows, setPagination, navigate]);

  const getQueryOptions = useCallback(
    (params: { page: number; limit: number }) => transferQueries.list({ ...listParams, ...params }),
    [listParams],
  );
  ...
  if (isError && !tList) {
    return (
      <TableErrorState
        title="Inventory Transfers unavailable"
  ...
  return (
    <div className="h-full w-full overflow-hidden bg-white flex flex-col">
      <TransferLookupLayer tableId={TABLE_ID} table={table} onReset={handleResetTable} />
