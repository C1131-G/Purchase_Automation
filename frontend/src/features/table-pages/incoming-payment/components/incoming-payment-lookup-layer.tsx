...
export function IncomingPaymentLookupLayer({
  ...
  const docNumSuggestionsQuery = useQuery(incomingPaymentQueries.docNumSuggestions(undefined, 10))
  ...
  const docNumLookupSearchQuery = useQuery({
    ...incomingPaymentQueries.docNumSuggestions(docNumLookupSearchTerm || undefined),
    enabled: shouldQueryDocNumSearch,
  })

  useEffect(() => {
    if (!lookupPopupOpen || lookupColumnId !== 'DocNum') return
    void queryClient.prefetchQuery(incomingPaymentQueries.docNumSuggestions(undefined, 100))
