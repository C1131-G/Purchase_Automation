...
export function APCreditNoteLookupLayer({
  ...
  const docNumSuggestionsQuery = useQuery(apCreditNoteQueries.docNumSuggestions(undefined, 10))
  ...
  const docNumLookupSearchQuery = useQuery({
    ...apCreditNoteQueries.docNumSuggestions(docNumLookupSearchTerm || undefined),
    enabled: shouldQueryDocNumSearch,
  })

  useEffect(() => {
    if (!lookupPopupOpen || lookupColumnId !== 'DocNum') return
    void queryClient.prefetchQuery(apCreditNoteQueries.docNumSuggestions(undefined, 100))
