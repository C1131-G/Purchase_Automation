...
const TRANSFER_REQUEST_BREADCRUMB = {
  href: "/inventory/transfer-request",
  page: "Transfer Request Data Table",
  section: "Inventory",
} as const;
...
const transferRequestFilterValueResolver = (item: LookupItem): string => {
  return item.code;
};

export interface TransferRequestLookupLayerProps {
  tableId: string;
  table: ReturnType<typeof useReactTable<TransferRequestListItem>>;
  onReset: () => void;
}

export function TransferRequestLookupLayer({
  tableId,
  table,
  onReset,
}: TransferRequestLookupLayerProps) {
  ...
  const {
    ...
    filterValueResolver: transferRequestFilterValueResolver,
  ...
  const docNumSuggestionsQuery = useQuery(
    transferRequestQueries.docNumSuggestions(undefined, DOC_NUM_QUICK_LIMIT),
  );
  const docNumSuggestionsBackgroundQuery = useQuery({
    ...transferRequestQueries.docNumSuggestions(undefined, DOC_NUM_BACKGROUND_LIMIT),
  ...
  const docNumLookupSearchQuery = useQuery({
    ...transferRequestQueries.docNumSuggestions(
  ...
  return (
    <>
      <TableToolbar
        ...
        table={table}
        onReset={onReset}
        createLink="/inventory/transfer-request/create"
        breadcrumb={TRANSFER_REQUEST_BREADCRUMB}
      ...
      <LookupPopup
        ...
        search={lookupSearch}
        results={lookupColumnId === "DocNum" ? docNumLookupResults : warehouses}
        loading={
        ...
        title={lookupColumnId === "DocNum" ? "Search Doc Number" : "Select Warehouse"}
        searchPlaceholder={
          lookupColumnId === "DocNum" ? "Search document number" : "Search warehouse code or name"
        }
