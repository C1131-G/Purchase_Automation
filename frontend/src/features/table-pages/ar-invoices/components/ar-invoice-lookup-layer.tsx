...
export function ARInvoiceLookupLayer({
  ...
  const docNumSuggestionsQuery = useQuery(arInvoiceQueries.docNumSuggestions(undefined, 10))
  ...
  const docNumLookupSearchQuery = useQuery({
    ...arInvoiceQueries.docNumSuggestions(docNumLookupSearchTerm || undefined),
    enabled: shouldQueryDocNumSearch,
  })

  useEffect(() => {
    if (!lookupPopupOpen || lookupColumnId !== 'DocNum') return
    void queryClient.prefetchQuery(arInvoiceQueries.docNumSuggestions(undefined, 100))
