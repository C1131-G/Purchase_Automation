...
export function ARCreditNoteLookupLayer({
  ...
  const docNumSuggestionsQuery = useQuery(arCreditNoteQueries.docNumSuggestions(undefined, 10))
  ...
  const docNumLookupSearchQuery = useQuery({
    ...arCreditNoteQueries.docNumSuggestions(docNumLookupSearchTerm || undefined),
    enabled: shouldQueryDocNumSearch,
  })

  useEffect(() => {
    if (!lookupPopupOpen || lookupColumnId !== 'DocNum') return
    void queryClient.prefetchQuery(arCreditNoteQueries.docNumSuggestions(undefined, 100))
