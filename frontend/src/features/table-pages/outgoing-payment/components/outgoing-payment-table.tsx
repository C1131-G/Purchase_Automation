...
import { createOutgoingPaymentColumns } from "@/features/table-pages/outgoing-payment/components/outgoing-payment-columns";
import { OutgoingPaymentLookupLayer } from "@/features/table-pages/outgoing-payment/components/outgoing-payment-lookup-layer";
import { outgoingPaymentColumnFilterSchema } from "@/features/table-pages/outgoing-payment/schemas/outgoing-payment-search.schema";
import type {
...
// OutgoingPaymentTable: Primary interface for tracking accounts payable disbursements.
// Leverages URL search params for a robust, shareable list experience.
export function OutgoingPaymentTable() {
  ...
  // React Query Integration: Aggressively fetches and caches list data based on URL coordinates.
  const {
    data: paymentList,
    isLoading,
    isFetching,
  ...
  // Coordination Layer: Maps complex URL search states to simple table-rendered views.
  ...
  const getQueryOptions = useCallback(
    (params: { page: number; limit: number }) =>
      outgoingPaymentQueries.list({ ...listParams, ...params }),
    [listParams],
  );
