...
export function DateFilterSearch<TData>({
  ...
  const { selectedRange, previewRange, label } = useMemo(() => {
    ...
    if (previewRange.from) {
      sRange.from = new Date(`${previewRange.from}T00:00:00`);
    }
    if (previewRange.to) {
      sRange.to = new Date(`${previewRange.to}T00:00:00`);
    }
    ...
    return { label: displayLabel, previewRange, selectedRange: sRange };
