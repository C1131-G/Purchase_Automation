...
const COLUMNS = [
  { key: "itemNo", label: "Item No.", width: "10%" },
  { key: "itemDescription", label: "Item Description", width: "20%" },
  { key: "uomCode", label: "UoM Code", width: "8%" },
  { key: "uomName", label: "UoM Name", width: "8%" },
  { key: "whse", label: "Whse", width: "6%" },
  { key: "quantity", label: "Quantity", width: "7%" },
  { key: "unitPrice", label: "Unit Price", width: "9%" },
  { key: "total", label: "Total", width: "9%" },
  { key: "binLocationAllocation", label: "Bin Location Allocation", width: "11%" },
  { key: "accountCode", label: "Account Code", width: "8%" },
  { key: "itemCost", label: "Item Cost", width: "8%" },
];

export function GoodsReceiptTable({ rows, onRowsChange }: GoodsReceiptTableProps) {
  // calculate total of row: quantity * parsed unit price
  const getRowTotal = (row: GoodsReceiptRow) => {
    ...
    return computedTotal > 0 ? `FJD ${computedTotal.toFixed(2)}` : "";
  };
  ...
  const updateRowField = (id: string, field: keyof GoodsReceiptRow, value: any) => {
    const updated = rows.map((r) => {
      if (r.id !== id) return r;
      const newRow = { ...r, [field]: value };
      newRow.total = getRowTotal(newRow);
      return newRow;
    });
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-3 space-y-3">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1100px] text-left text-xs text-zinc-700">
                  ...
                  <tr key={row.id} className="hover:bg-zinc-50/50 transition duration-150">
                    ...
                    <td className="px-2 py-2">
                      <input
                        ...
                        className="h-9 w-full rounded-lg border border-transparent bg-zinc-50 px-2 text-xs text-zinc-800 outline-none transition hover:border-zinc-200 focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-200 text-right"
                        value={row.unitPrice}
                        onChange={(e) => updateRowField(row.id, "unitPrice", e.target.value)}
                        placeholder="FJD 0.00"
                        aria-label={`Unit Price row ${idx + 1}`}
                      />
                    </td>

                    {/* Total */}
                    <td className="px-2 py-2 text-right font-medium text-zinc-900 pr-4">
                      {rowTotal || "FJD 0.00"}
                    </td>
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <div className="text-right pr-4">
          <span className="text-xs font-medium text-zinc-500 mr-2">Grand Total:</span>
