...
import type { GoodsReceiptRow } from "../types/goods-receipt.types";

interface GoodsReceiptProductTableRowProps {
  row: GoodsReceiptRow;
  openProductPopup: (rowId: string | null) => void;
  updateProductRow: (id: string, patch: Partial<GoodsReceiptRow>) => void;
...
export function GoodsReceiptProductTableRow({
  row,
  openProductPopup,
  updateProductRow,
  removeProductRow,
  ...
  const handlePriceQuantityChange = (field: "quantity" | "unitPrice", value: string | number) => {
    const patch: Partial<GoodsReceiptRow> = { [field]: value };
