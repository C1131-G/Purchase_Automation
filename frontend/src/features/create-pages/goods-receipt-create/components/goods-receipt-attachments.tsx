...
interface GoodsReceiptAttachmentsProps {
  attachments: AttachmentItem[];
  onAttachmentsChange: (items: AttachmentItem[]) => void;
}

export function GoodsReceiptAttachments({
  attachments,
  onAttachmentsChange,
}: GoodsReceiptAttachmentsProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      newAttachments.push({
        id: `${Date.now()}-${i}`,
        targetPath: `C:\\SAP_Attachments\\${file.name}`,
        fileName: file.name,
        attachmentDate: new Date().toLocaleDateString("en-GB"), // format DD/MM/YYYY
        freeText: "",
      });
    }
