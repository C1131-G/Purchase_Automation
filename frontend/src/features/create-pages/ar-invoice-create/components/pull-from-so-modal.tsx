...
interface PullFromSOModalProps {
  ...
  onClose: () => void;
  cardCode: string;
  onConfirm: (selectedLines: OpenSalesOrderLine[]) => void;
...
export function PullFromSOModal({
  open,
  onClose,
  cardCode,
  onConfirm,
  ...
  const toggleSelectAll = () => {
    ...
    if (allSelectableSelected) {
      // Deselect only the non-committed ones
      const next = new Set(selectedDocNums);
  ...
  const handleConfirm = () => {
    // Only pull lines for newly selected docs — skip committed ones to avoid duplicates.
    ...
    <AnimatedModalShell open={open} onClose={onClose} panelClassName="max-w-4xl">
      ...
      <div className="p-6">
        <div className="relative mb-4">
          ...
          <input
            type="text"
            placeholder="Search by SO # or Product..."
                  ...
                  <tr>
                    <td colSpan={6} className="py-20 text-center">
                      <div className="flex flex-col items-center gap-1">
                        <p className="font-medium text-zinc-500">No open sales orders found</p>
