...
interface UsePoLookupsProps {
  headerVendorCode: string;
  headerVendorName: string;
...
export function usePoLookups({
  ...
  const selectVendor = (vendor: LookupOption) => {
    if (normalizeCodeForCompare(headerVendorCode) === normalizeCodeForCompare(vendor.code)) {
      setPendingVendorChange(null);
      setNameInput(headerVendorName || vendor.name);
      setCodeInput(headerVendorCode || vendor.code);
  ...
  const cancelVendorChange = () => {
    setPendingVendorChange(null);
    setNameInput(headerVendorName);
    setCodeInput(headerVendorCode);
    vendorSelectedRef.current = true;
    setNameFocused(false);
    setCodeFocused(false);
  };
  ...
  }, [clearFieldError, salesEmployeeFocused, salesEmployeeInput]);
