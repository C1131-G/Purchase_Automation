...
interface UseArCnProductsProps {
...
export function useArCnProducts({
  ...
  const productWarehouseStocksQuery = useQuery({
    ...sharedQueries.productWarehouseStocks(stockPreviewProductCode),
  ...
  const openProductPopup = (
    ...
    window.requestAnimationFrame(() => {
      document
        .getElementById('ar-credit-memo-product-section')
  ...
  const applyProductsToRows = (
    ...
    products.forEach((product) => {
      void queryClient.prefetchQuery(sharedQueries.productWarehouseStocks(product.code))
    })
