...
import { notifyCreateApiError } from "@/features/create-pages/create-shared/utils/create-feedback-toast";
import type {
  CreateLookupOption,
  PopupMode,
...
export function useArCreditMemoCreate({
  ...
  const [pendingVendorChange, setPendingVendorChange] = useState<{ vendor: any } | null>(null);
  ...
  const selectVendor = (item: {
    ...
    if (hasCopiedRows) {
      setPendingVendorChange({ vendor: item });
      return;
    }
    applyVendorChange(item);
  };

  const applyVendorChange = (item: {
    code: string;
    name: string;
    billToAddress?: string | undefined;
    shipToAddress?: string | undefined;
  ...
  const confirmVendorChange = () => {
    ...
    const cleanedRemarks = stripAutoGeneratedReferences(header.comments ?? "");
  ...
  const handleCreateOrder = async (
    ...
    try {
      ...
      notifyCreateApiError(errorMessage, "ar-credit-memo");
    }
  };
