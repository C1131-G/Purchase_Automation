...
function rankResults(docs: DocumentLookupItem[], term: string) {
  ...
  const scored = docs.map((doc) => {
    ...
    if (numLower === lower) return { doc, rank: 0 }
    if (numLower.startsWith(lower)) return { doc, rank: 1 }
    if (numLower.includes(lower)) return { doc, rank: 2 }
    if (labelLower.includes(lower)) return { doc, rank: 3 }
    return { doc, rank: 4 }
  })
    ...
    <AnimatedModalShell open={open} onClose={onClose} panelClassName="max-w-lg">
      <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3">
        <div className="flex items-center gap-3">
          <h3 className="text-sm font-semibold text-zinc-900">Select Documents</h3>
          {selectedCount > 0 && (
      ...
      <div className="p-4">
        <div className="relative mb-3">
          ...
          <input
            className="h-10 w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 pl-10 pr-10 text-sm outline-none transition focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-200"
          ...
          {search && (
            <button
              type="button"
              className="pointer-events-auto absolute inset-y-0 right-0 flex items-center pr-3 text-zinc-400 hover:text-zinc-600"
