...
export function PaymentModal({ open, onClose, balanceDue, onPaymentSubmit }: PaymentModalProps) {
  ...
  const getActiveAmount = () => {
    ...
    if (activeTab === 'Card') return cardAmount
    return chequeAmount
  }

  const setActiveAmount = (val: string) => {
    if (activeTab === 'Cash') setCashAmount(val)
    if (activeTab === 'Card') setCardAmount(val)
    if (activeTab === 'Cheque') setChequeAmount(val)
  }
  ...
  let surchargeRate = 0
  if (cardType === 'VISA' || cardType === 'MASTERCARD') surchargeRate = 3.82
  else if (cardType === 'AMEX') surchargeRate = 4.91
  else surchargeRate = 0
            ...
            <div className="w-full space-y-4 animate-in fade-in pt-2">
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  { label: 'Branch', type: 'text', value: chequeBranch, setter: setChequeBranch },
                  { label: 'Check No.', type: 'text', value: chequeNo, setter: setChequeNo },
                  {
                    label: 'Account No.',
                    type: 'text',
