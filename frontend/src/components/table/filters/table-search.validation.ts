...
export const DOC_TOTAL_MAX = 999999999

const numberOnlySchema = z.string().regex(/^\d*$/).max(NUMBER_ONLY_MAX_LENGTH)
const alphanumericSchema = z.string().regex(/^[a-zA-Z0-9]*$/).max(ALPHANUMERIC_MAX_LENGTH)
const lettersSymbolsSchema = z.string().regex(/^[^0-9]*$/).max(LETTERS_SYMBOLS_MAX_LENGTH)
