...
const isNumberComparisonFilter = (value: unknown): value is NumberComparisonFilter => {
    ...
    const operatorValid = candidate.operator === 'eq' || candidate.operator === 'lt' || candidate.operator === 'gt'
...
export function TableSearch<TData>({
    ...
    const [draftNumberFilter, setDraftNumberFilter] = useState<{ operator: NumberComparisonOperator; value: string }>({
        operator: 'eq',
        value: '',
    })
        ...
        if (activeColumn) {
            ...
            if (filterType === 'number-comparison') {
                ...
                setDraftNumberFilter((prev) => {
                    ...
                    // Keep the currently selected operator; only clear the input value.
                    return {
            ...
            setSearchValue(currentFilterValue === undefined || currentFilterValue === null ? '' : String(currentFilterValue))
        } else {
            setSearchValue('')
                ...
                >
                    ...
                    <Select.Portal>
                        <Select.Positioner>
                            <Select.Popup>
                                <Select.List>
                                    <Select.Item value="eq" label={NUMBER_OPERATOR_LABEL.eq}>Equal (=)</Select.Item>
                                    <Select.Item value="lt" label={NUMBER_OPERATOR_LABEL.lt}>Less than (&lt;)</Select.Item>
                                    <Select.Item value="gt" label={NUMBER_OPERATOR_LABEL.gt}>Greater than (&gt;)</Select.Item>
                                </Select.List>
                            </Select.Popup>
                        </Select.Positioner>
                    </Select.Portal>
                            ...
                            <Select.Popup>
                                <Select.List>
                                    ...
                                    {filterOptions?.map((option) => {
                                        ...
                                        let icon = null
                                        if (labelText === 'Open') icon = <div className="size-2 rounded-full bg-emerald-500" />
                                        if (labelText === 'Closed') icon = <div className="size-2 rounded-full bg-zinc-400" />
                                        if (labelText === 'Yes (Canceled)') icon = <svg className="size-4 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                                        if (labelText === 'No (Active)') icon = <svg className="size-4 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
    ...
    return (
        <div className={cn("relative flex-1 max-w-sm group", className)}>
            ...
            <Input
                ...
                inputMode={activeColumn && NUMBER_ONLY_COLUMN_IDS.has(activeColumn.id) ? "numeric" : undefined}
                pattern={
