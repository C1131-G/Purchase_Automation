...
export function TableColumnOrder<TData>({ tableId, table, search, onToggleComplete }: TableColumnOrderProps<TData>) {
    ...
    const columns = table.getAllLeafColumns().filter(
        (column) => column.id !== 'actions' && !!column.columnDef.header
    )
    ...
    return (
        <div
            ...
            className={cn(
                ...
                isDragging ? 'bg-white shadow-md border-zinc-200 ring-1 ring-blue-100' : 'hover:bg-zinc-50 text-zinc-900'
            )}
        >
