...
const isCalendarDateRange = (value: Date | CalendarDateRange | undefined): value is CalendarDateRange =>
...
function Calendar({
    ...
    const [internalRange, setInternalRange] = useState<CalendarDateRange>(isCalendarDateRange(selected) ? selected : {})
        ...
        >
            <div className="flex items-center justify-between mb-4 relative z-10">
                ...
                <button
                    ...
                    disabled={view === 'picker' || normalizeDate(new Date(year, month, 1)) >= currentMonthBoundary}
                >
                    <ChevronRight size={20} className={cn((view === 'picker' || normalizeDate(new Date(year, month, 1)) >= currentMonthBoundary) && 'opacity-30')} />
                </button>
            </div>
            ...
            <div className="h-[228px] relative">
                <div
                    className={cn(
                        ...
                        view === 'grid' ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95 pointer-events-none',
                    )}
                    style={{
                    ...
                    <div className="flex flex-col h-full">
                        ...
                        <div className="grid grid-cols-7 gap-y-1 flex-1 content-start">
                            {days.map((day, i) => {
                                ...
                                const isSelectedSingle = mode === 'single' && internalSingle ? sameDay(d, internalSingle) : false
                                ...
                                const isRangeMiddle = Boolean(from && to && !isRangeEdge && isBetweenInclusive(d, from, to))

                                return (
                                    <div key={`day-${day}`} className="flex items-center justify-center aspect-square">
                                        <button
                                            type="button"
                                            ...
                                            className={cn(
                                                ...
                                                (isSelectedSingle || isRangeStartOnly || isRangeEdge) && 'bg-blue-50 border border-blue-200 text-blue-700 font-bold shadow-sm',
                                                isRangeMiddle && 'bg-blue-50/50 text-blue-500 font-semibold',
                                                !isFuture && !isSelectedSingle && !isRangeStartOnly && !isRangeEdge && !isRangeMiddle && 'text-zinc-800 font-medium',
                                            )}
                                            style={{
                ...
                <div
                    className={cn(
                        'absolute inset-0 transition-all transform',
                        view === 'picker' ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 -translate-y-8 scale-95 pointer-events-none',
                    )}
                    style={{
                        transitionDuration: `${MOTION_MS.calendarView}ms`,
                        transitionTimingFunction: MOTION_EASING.smoothOut,
                ...
                >
                    <div className="flex h-full relative">
                        ...
                        <div ref={monthScrollRef} className="flex-1 overflow-y-auto py-[120px] scroll-smooth" style={{ scrollbarWidth: 'none' }}>
                                ...
                                {MONTHS.map((m, index) => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            year === today.getFullYear() && index > today.getMonth() && 'text-zinc-300 cursor-not-allowed',
                        ...
                        <div ref={yearScrollRef} className="flex-1 overflow-y-auto py-[120px] scroll-smooth" style={{ scrollbarWidth: 'none' }}>
                            <div className="flex flex-col items-center gap-2">
                                {years.map((y, _index) => (
                                    <button
                                        key={y}
