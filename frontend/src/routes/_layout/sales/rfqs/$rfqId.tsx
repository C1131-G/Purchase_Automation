...
/**
 * Phase 1 stub — Phase 2 replaces this with a PQ-create-mirrored RFQ fill form.
 * Keeps double-click navigation from the RFQ table stable.
 */
export const Route = createFileRoute("/_layout/sales/rfqs/$rfqId")({
  component: RfqDetailStubRoute,
});

function RfqDetailStubRoute() {
  ...
  useDocumentTitle(`RFQ ${titleNumber} | ERP Portal`);
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-3xl flex-col gap-4 px-6 py-8">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
        <h1 className="text-lg font-semibold tracking-tight text-zinc-950">RFQ {titleNumber}</h1>
        <p className="mt-2 text-sm text-zinc-600">
          Detail and fill form ship in Phase 2 (layout mirrored from Purchase Quotation create).
          Table open/navigation is ready.
        </p>

        {detailQuery.isLoading ? <p className="mt-4 text-sm text-zinc-500">Loading RFQ…</p> : null}
