...
export const Route = createFileRoute("/_layout/purchase/grpo-lots/batches")({
  component: BatchesSetupRoute,
  pendingComponent: CreatePageRouteSkeleton,
  validateSearch: searchSchema,
});

function BatchesSetupRoute() {
  useDocumentTitle("Batches - Setup | ERP Portal");
  const { selectedRowId } = Route.useSearch();
  const lines = useGRPOLines();
  const returnTo = useGRPOLotSessionStore((state) => state.returnTo);

  if (!hasLotKindRows(lines, "batches")) {
    return (
      <Navigate
        replace
  ...
  return <LotSetupPage kind="batches" {...(selectedRowId ? { selectedRowId } : {})} />;
}
