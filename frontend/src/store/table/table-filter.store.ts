...
export const useTableFilterStore = create<FilterStore>((set) => ({
  ...
  setAllowNextFilterPopoverOpen: (tableId, allow) =>
    set((prev) => {
      const current = prev.tables[tableId] || DEFAULT_FILTER_STATE
      return {
        tables: { ...prev.tables, [tableId]: { ...current, allowNextFilterPopoverOpen: allow } },
      }
    }),
...
export const useInitFiltersAction = () => useTableFilterStore((state) => state.initFilters)

export const useResetFiltersAction = () => useTableFilterStore((state) => state.resetFilters)
