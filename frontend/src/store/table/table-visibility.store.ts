...
interface VisibilityStore {
    ...
    setVisibility: (tableId: string, visibility: Record<string, boolean> | ((prev: Record<string, boolean>) => Record<string, boolean>)) => void
...
export const useTableVisibilityStore = create<VisibilityStore>((set) => ({
    tables: {},
    initVisibility: (tableId, visibility) => set((prev) => {
        if (prev.tables[tableId]) return prev
        return { tables: { ...prev.tables, [tableId]: visibility } }
    }),
    setVisibility: (tableId, visibility) => set((prev) => {
    ...
    resetVisibility: (tableId) => set((prev) => ({
...
export const useSetVisibilityAction = () =>
    useTableVisibilityStore((state) => state.setVisibility)
