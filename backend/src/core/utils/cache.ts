...
export const getCachedData = async <T>(
  ...
  if (inFlight.has(key)) {
    logger.debug({ msg: "Cache WAIT (In-flight)", key });
...
export const invalidateKey = (key: string): void => {
  cache.delete(key);
  logger.debug({ msg: "Cache invalidated by key", key });
};
...
export const purgeCache = (pattern: string): void => {
  ...
  if (count > 0) {
    logger.info({ msg: "Cache purged by pattern", pattern, removedCount: count });
  }
};
