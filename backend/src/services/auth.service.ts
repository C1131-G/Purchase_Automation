...
export const login = async (
  ...
  try {
    ...
    logger.info({ msg: "User login success", username, company: dbName });
    ...
    const sapError = error as ExtendedSLError;
    logger.error({
...
export const logout = async (sessionId: string): Promise<void> => {
  try {
    ...
    const error = err instanceof Error ? err : new Error(String(err));
    logger.warn({ msg: "SAP logout failed (session may have expired)", error: error.message });
  }
};
...
export const getSessionInfo = (sessionId: string) => {
  return serviceLayerClient.getSession(sessionId);
};
