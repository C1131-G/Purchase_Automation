...
// Creates a formal A/P Credit Note in SAP. Handles payload conversion and file attachments.
export const createCreditNote = async (
  ...
  try {
    ...
    // Process file attachments through the Service Layer's Attachment service.
    ...
    if (uploadedFilesRaw) {
      ...
      if (uploadedFiles.length > 0) {
        try {
          ...
          // If attachment fails, log it but don't prevent credit note creation.
