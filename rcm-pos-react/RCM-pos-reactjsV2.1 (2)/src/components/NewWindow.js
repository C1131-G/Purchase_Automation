...
class NewWindow extends React.PureComponent {
  ...
  static defaultProps = {
    ...
    name: "",
    title: "",
    features: { width: "700px", height: "640px" },
    onBlock: null,
    onOpen: null,
    onUnload: null,
    center: "parent",
  ...
  openChild() {
    ...
    if (this.window) {
      ...
      /** NOTE:
       * Tried calling this.window.print() from
       *     - componentDidMount
       *     - componentDidUpdate
       *     - via addEventListener('load',..)
       *     - & here in this block below 'addEventListener('beforeunload',..) code
       * but they all just printed blank page, bcoz the print() method got invoked b4 the qr codes
       * were fully loaded in the new window. Could find a way to automatically call print(),
       * So finally added a button in teh New Window to print the page once it is fully loaded
       */

      //Added cond. to hide the Print Button when the page already has its own `Print` btn.
      if (!this.props.hidePrintButton) {
        //Add Print button at the top of the New Window
        const button = document.createElement("button");
        button.type = "button";
        button.innerHTML = "Click to Print";
        button.className = "btn btn-sm btn-primary donot-printme";
        button.setAttribute("style", "position: fixed; top: 20px; right: 25px");
        button.onclick = () => {
          this.window.print();
          this.window.close();
        };
        this.window.document.body.prepend(button);
      }
