//StockTransferRequestList.6 b4 'Resubmit' Rejectd rec..js
...
class StockTransferRequestList extends React.Component {
  state = {
    ...
    allStockTransferRequestList: [],
    filteredStockTransferRequestList: [],
    sort: {
      column: null,
      direction: "desc",
    },
  ...
  /**
   * Gets all GRPODraft details from API and sets them to state variables
   */
  getTransferRequestRecords = async () => {
    console.log(
      "Index - getTransferRequestRecords() this.state.draftStatus: " + this.state.draftStatus,
    );
    this.setState({ isLoading: true });
    let filteredStockTransferRequestList = [];
    ...
    try {
      const response = await api.get("/custom/stock-transfer-request", {
        params: { userRole: this.props.userRole, userId: localStorage.getItem("InternalKey") },
      });
      console.log(`getTransferRequestRecords response.data: ${JSON.stringify(response.data)}`);
      if (Array.isArray(response.data) && response.data.length) {
        if (draftStatus === "ALL" || !draftStatus) filteredStockTransferRequestList = response.data;
        else {
          filteredStockTransferRequestList = response.data.filter((rec) => {
            return rec.U_DraftStatus == this.state.draftStatus;
          });
        }
        this.setState({
          allStockTransferRequestList: response.data,
          filteredStockTransferRequestList,
  ...
  handleStatusChange = (event) => {
    ...
    const { allStockTransferRequestList } = this.state;
    let filteredStockTransferRequestList = [];
    if (draftStatus === "ALL") filteredStockTransferRequestList = allStockTransferRequestList;
    else {
      filteredStockTransferRequestList = allStockTransferRequestList.filter((rec) => {
    ...
    this.setState({
      draftStatus,
      filteredStockTransferRequestList,
      count: filteredStockTransferRequestList.length,
    });
  };
  ...
  handleSearch = (searchKey) => {
    ...
    allStockTransferRequestList.forEach((request) => {
      if (
        request.DocEntry.toString().indexOf(searchKey) > -1 ||
        ...
        filteredStockTransferRequestList.push(request);
      }
    });
    //console.log(`filteredStockTransferRequestList: ${JSON.stringify(filteredStockTransferRequestList)}`);
    this.setState({
      filteredStockTransferRequestList,
      count: filteredStockTransferRequestList.length,
      draftStatus: "ALL",
  ...
  handleSort = (column) => (e) => {
    ...
    if (column === "DocDate") column = "DocEntry";
    ...
    const direction = this.state.sort.column
      ? this.state.sort.direction === "asc"
        ...
        : "asc"
      : "desc";
    const sortedData = this.state.filteredStockTransferRequestList.sort((a, b) => {
    ...
    this.setState({
      filteredStockTransferRequestList: sortedData,
      sort: {
  ...
  async componentDidMount() {
    console.log("StockTransferRequestList - componentDidMount userRole: " + this.props.userRole);
    console.log(
      "StockTransferRequestList - componentDidMount draftStatus- " + this.props.draftStatus,
    );
    /**
    ...
    if (this.state.operation !== "Cancel") {
      await this.getTransferRequestRecords();
    }
  }
  ...
  render() {
    ...
    if (isModuleApprovalsEnabled) {
      ...
      if (this.props.userRole === userRoles.APPROVER) {
        ...
        requestTableHeader.push({ label: "No. Of Approvals", sortField: "U_NoOfApprovals" });
      }
    }
                ...
                <Card className="bg-white shadow">
                  <CardHeader className="border-0 mb--4 mt--2">
                        ...
                        <div className="mb-2 mt--2">
                          ...
                          <small>
                            {this.props.userRole === userRoles.ORIGINATOR
                              ? "List of Stock Transfer Requests you submitted. Click on a Request# to view the request details."
                              : "List of Stock Transfer Requests awaiting your approval. Click on the Request# to view the request details."}
                          </small>
                        </div>
                      </Col>
                    ...
                    <Row className="text-left mb-3">
                      ...
                      <Col md="2" className="mt-4 pt-1 text-right">
                        <Button
                          size="sm"
                          color="primary"
                          type="button"
                          onClick={() =>
                            this.props.setGRPODraftDetails("Create", null, 3, this.props.userRole)
                          }
                        >
                          Create New Request
                        </Button>
                ...
                */}
                  <Card className="table-fixed-head table-fixed-head-lg">
                    <Table size="sm" className="align-items-center table-flush" responsive>
                      <thead className="thead-light">
                        <tr>
                          {requestTableHeader.map((headerCol, key) => {
                            return headerCol.label !== colName ? (
                              <th
                                scope="col"
                                key={headerCol.label}
                                ...
                                style={{
                                  textAlign:
                                    headerCol.label === "Approver Details" ? "center" : "auto",
                                }}
                              >
                                {headerCol.label}
                              ...
                              <th
                                ...
                                key={headerCol.label}
                                onClick={
                                  colName === "Requestor" ? this.handleSort(colName) : undefined
                                }
                                style={{
                                  cursor: colName === "Requestor" && "pointer",
                                  textAlign: "center",
                                }}
                              >
                                {colName}
                      ...
                      <tbody>
                        {Array.isArray(this.state.filteredStockTransferRequestList) &&
                        this.state.filteredStockTransferRequestList.length ? (
                          this.state.filteredStockTransferRequestList.map((request, key) => {
                            return (
                              <tr key={request.DocEntry}>
                                <th className="mb-0 text-sm" scope="row">
                                  <a
                                    ...
                                    onClick={
                                      request.U_DraftStatus === draftStatusList.PENDING
                                        ? () =>
                                            this.props.setGRPODraftDetails(
                                              "Edit",
                                              request,
                                              3,
                                              this.props.userRole,
                                ...
                                {isModuleApprovalsEnabled && (
                                  <>
                                    <td style={{ width: "10%", textAlign: "left" }}>
                                      <StatusColumn
                                        status={request.U_DraftStatus}
                                        targetRecDocNum={request.U_TargetRecDocNum}
                                      />
                                    </td>
                                    <td>
